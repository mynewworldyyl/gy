...

{
	 ...
	 * 
	 * @param filename
	 *            file from which a fingerprint is build
	 */
	 ...
	 * 
	 * @param zeroColor
	 *            Color of zero (off) pixels
	 * @param zeroColor
	 *            Color of one (on) pixels
	 */
	 ...
	 * 
	 * @param core
	 *            the core point (center of fingerprint)
	 * @param coreRadius
	 *            radius from core that defines the considered area
		...
		for (int i = minI; i < maxI; ++i) {
			for (int j = minJ; j < maxJ; ++j) {
				...
				if (getDistance(currentPoint, core) < coreRadius) {
					...
					if ((binMap[i][j] == true)
							&& ((nbOnNeighbors == 3) || (nbOnNeighbors == 4))) {
			...
			for (int j = minJ; j < maxJ; ++j) {
				...
				if (getDistance(currentPoint, core) < coreRadius) {
					...
					if ((binMap[i][j] == true)
							...
							&& ((neighbors[0] == true)
									|| (neighbors[2] == true)
									|| (neighbors[4] == true) || (neighbors[6] == true))) {
						minutiae.add(currentPoint);
					}
				}
			}
	 ...
	 * 
	 * @param size
	 *            number of pixels to add (or remove if < 0)
	 */
	public void addBorders(int size) {
		...
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				if (i < limitLeft || i > limitRight || j < limitTop
						|| j > limitBottom) {
					newmap[i][j] = false;
				} else {
	...
	public void removeNoise() {
		...
		float[][] filter = { { 0.0625f, 0.1250f, 0.0625f },
				{ 0.1250f, 0.2500f, 0.1250f }, { 0.0625f, 0.1250f, 0.0625f } };
			...
			for (int j = 1; j < limHeight; ++j) {
				...
				for (int ik = -1; ik <= 1; ++ik) {
					for (int jk = -1; jk <= 1; ++jk) {
						val += booleanToInt(binMap[i + ik][j + jk])
								* filter[1 + ik][1 + jk];
					}
				}
				newmap[i][j] = (val > 0.5);
		...
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				bufferedImage.setRGB(i, j,
						(dirToColor(dirMatrix[i][j])).getRGB());
			}
		}
	...
	public Point getCore(direction[][] dirMatrix) {
		...
		class coreInfos {
			...
			public float getIndex() {
				...
				res = Math.abs(perVer - .25f) + Math.abs(perHor - .25f)
						+ Math.abs(perPos - .25f) + Math.abs(perNeg - .25f);
	...
	private float getDistance(Point a, Point b) {
		...
		return (float) Math.sqrt(Math.abs(deltaX * deltaX)
				+ Math.abs(deltaY * deltaY));
	}
	 ...
	 *            second matrix
	 * @param w
	 *            width of the two matrices
	 * @param h
	 *            height of the two matrices

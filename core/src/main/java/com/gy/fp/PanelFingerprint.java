...
public class PanelFingerprint extends JPanel {
	...
	public PanelFingerprint() {
		...
		// Load images
		loadingIcon = Toolkit.getDefaultToolkit().getImage(
				"./ressources/loading.gif");
		coreIcon = Toolkit.getDefaultToolkit()
				.getImage("./ressources/core.png");
	}
	...
	/**
	 * Set the fingerprint image as working or not. When working, the panel
	...
	protected void paintComponent(Graphics g) {
		...
		if (isWorking) {
			...
			g2d.drawImage(loadingIcon, Math.round(getWidth() * .5f) - 16,
					Math.round(getHeight() * .5f) - 16, 32, 32, this);
			return;
		}
		...
		// Draw the core
		if (core != null) {
			buffer.getGraphics().drawImage(coreIcon, core.x - 16, core.y - 16,
					32, 32, this);

			buffer.getGraphics().setColor(Color.white);
			buffer.getGraphics().drawOval(core.x - coreRadius,
					core.y - coreRadius, 2 * coreRadius, 2 * coreRadius);
		}
